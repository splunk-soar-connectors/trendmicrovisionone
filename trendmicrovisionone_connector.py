...
class TrendMicroVisionOneConnector(BaseConnector):
    ...
    def update_container(self, i, old_container):
        ...
        try:
            requests.post(url, data=json.dumps(update), verify=False)  # nosemgrep
        except Exception:
    ...
    def artifact_exists(self, sdi, container):
        ...
        try:
            ...
            response = requests.get(url, verify=False)  # nosemgrep
        except Exception:
            return None
        # return id or None
    ...
    def _handle_status_check(self, param):
        ...
        while timer > 0 and 'success' not in response.get("data").get("taskStatus"):
            ...
            elapsed += 5
            # abort if failed
            if phantom.is_fail(ret_val) or 'failed' in response.get("data").get("taskStatus"):
    ...
    def _handle_remove_from_blocklist(self, param):
        ...
        # make rest call
        ret_val, response = self._make_rest_call(
            REMOVE_BLOCKLIST_ENDPOINT, action_result, method='post', data=json.dumps(body), headers=self.header()
        )
    ...
    def _handle_delete_email_message(self, param):
        ...
        # make rest call
        ret_val, response = self._make_rest_call(
            DELETE_EMAIL_ENDPOINT, action_result, method='post', data=json.dumps(body), headers=self.header()
        )
    ...
    def _handle_terminate_process(self, param):
        ...
        # make rest call
        ret_val, response = self._make_rest_call(
            TERMINATE_PROCESS_ENDPOINT, action_result, method='post', data=json.dumps(body), headers=self.header()
        )
    ...
    def _handle_add_to_exception_list(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def _handle_add_to_suspicious_list(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def _handle_delete_from_suspicious_list(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def _handle_get_file_analysis_status(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def file_to_vault(self, data, filename, container_id, action_result):
        ...
        try:
            ...
            self.debug_print(e)
            return action_result.set_status(phantom.APP_ERROR, "Failed erasing temporary vault related data")
        ...
        return action_result.set_status(phantom.APP_ERROR, "Failed to send file to vault")

    def _handle_get_file_analysis_report(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
        ...
        if isinstance(response, dict):
            ...
            for i in message['data']:
                ...
                artifacts_d['source_data_identifier'] = 'File Analysis Report - Suspicious Object'
    ...
    def _handle_collect_file(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def _handle_download_information_collected_file(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def _handle_submit_file_to_sandbox(self, param):
        # send progress messages back to the platform
        self.save_progress("In action handler for: {0}".format(self.get_action_identifier()))
    ...
    def handle_action(self, param):
        ...
        action_dict = {
            ...
            'add_to_exception_list': self._handle_add_to_exception_list,
            'add_to_suspicious_list': self._handle_add_to_suspicious_list,
            'delete_from_suspicious_list': self._handle_delete_from_suspicious_list,
            'get_file_analysis_status': self._handle_get_file_analysis_status,
            'get_file_analysis_report': self._handle_get_file_analysis_report,
            'collect_file': self._handle_collect_file,
            'download_information_collected_file': self._handle_download_information_collected_file,
            'submit_file_to_sandbox': self._handle_submit_file_to_sandbox,
            'remove_from_blocklist': self._handle_remove_from_blocklist,
            'delete_email_message': self._handle_delete_email_message,
            'delete_from_exception_list': self._handle_delete_from_exception_list,
